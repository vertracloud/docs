---
title: All Applications Status
sidebarTitle: All Applications Status
api: GET https://api.vertracloud.app/v1/apps/status
authMethod: bearer
description: Retrieves the status of all applications for the authenticated user or a specified workspace.
---

{/* @see APIApplicationStatusShort[] — @vertracloud/api-types */}
<ParamField query="workspace_id" type="string">
  The workspace ID to retrieve application status, if applicable.
</ParamField>

### Response

<ResponseField name="response" type="array">
  An array of status objects for each application.

  <Expandable title="Toggle object">

  <ResponseField name="id" type="string">
    The application ID.
  </ResponseField>

  <ResponseField name="cpu" type="string">
    CPU usage percentage (e.g., `25.5%`).
  </ResponseField>

  <ResponseField name="ram" type="string">
    RAM usage (e.g., `512 MB`).
  </ResponseField>

  <ResponseField name="running" type="boolean">
    Indicates if the application is running.
  </ResponseField>

  <ResponseField name="installing" type="boolean">
    Present and `true` while dependencies are being installed or the application is rebuilding. During this time, `running` can be `false` even though work is in progress.
  </ResponseField>

  </Expandable>
</ResponseField>

<ResponseExample>

```json Response
{
  "response": [
    {
      "id": "app-abc123def456",
      "cpu": "25.50%",
      "ram": "512.00 MB",
      "running": true
    },
    {
      "id": "app-xyz789mno012",
      "cpu": "0.00%",
      "ram": "0.00 MB",
      "running": false
    }
  ]
}
```

</ResponseExample>

### Error Responses

<ResponseField name="code" type="string">
  The error code indicating the reason for the failure. Possible values:
  - `UNAUTHORIZED`: The user is not authenticated or does not have sufficient permissions.
  - `WORKSPACE_NOT_FOUND`: The specified workspace does not exist.
  - `USER_NOT_IN_WORKSPACE`: The user is not a member of the specified workspace.
</ResponseField>

<ResponseField name="message" type="string">
  A descriptive message providing additional details about the error.
</ResponseField>

<ResponseExample>

```json Error Response (403)
{
  "code": "UNAUTHORIZED"
}
```

```json Error Response (404)
{
  "code": "WORKSPACE_NOT_FOUND"
}
```

```json Error Response (403)
{
  "code": "USER_NOT_IN_WORKSPACE"
}
```

</ResponseExample>